...
func check_rotation(window *glfw.Window, rot *float32, body_x float32, body_y float32) {
	width, height := window.GetFramebufferSize()

	cursor_x, cursor_y := window.GetCursorPos()
	origin_x, origin_y := (width / 2), (height / 2)

	cat_x := (cursor_x - float64(origin_x)) - float64(body_x*float32(origin_x))
	cat_y := (float64(origin_y) - cursor_y) - float64(body_y*float32(origin_y))
